...
if [ -n "$input" ]; then
    echo "$input" > /tmp/timer_input_value
    # Обновляем переменную
    eww -c /home/ig_ro/Документы/eww/timer update timer_input="$input" 2>/dev/null
    # Ждем короткое время, чтобы обновление вступило в силу
    sleep 0.5
    # Повторно обновляем, чтобы гарантировать перерисовку
    eww -c /home/ig_ro/Документы/eww/timer update timer_input="$input" 2>/dev/null
fi